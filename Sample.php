...
<body>
    ...
    <span style="display:none;" class="close">&times;</span>
        ...
        <div class="form-group">
            ...
            <input type="text" id="auctionDate">
        </div>
        <div class="form-group">
    ...
    <button type="button" class="saveButton" style="display: none;">Save lots </button>
        ...
        <div class="form-group">
            ...
            <select id="type" name="type" required>
                ...
                <option value="Paper">Paper sack</option>
            </select>
        </div>
...
<?php include 'coverpage2.php' ?>;
    ...
    <h2 style="color:green">Auction Details</h2>
    <div id="dataContainer">
        <p id="defaultMessage" class="movingMessage">No auction number selected. Please select an auction number to view data.</p>

        <!-- Table 1 for Grades BP1, PF1, PD, D1 -->
        <h3 style="color:blue; text-align:center;" class="h3">Main Catalogue</h3>
        <table id="dataTable1">
        ...
        <h3 style="color:blue; text-align:center; margin-top:8px;" class="h3">Secondary Catalogue</h3>
        <table id="dataTable2">
            <thead class="thead">
                <tr>
                    <th style="text-align: center; color:green; font-weight:bold;" colspan="16">VISION TEA BROKERS LTD</th>
...
<button id="printButton" style="display:none; margin:20px;">Print</button>
<button id="pdfButton" style="display:none; margin:20px;">PDF</button>
<button id="excelButton" style="display:none; margin:20px;">Excel</button>
<button id="wordButton" style="display:none; margin:20px;">Word</button>
...
$(document).on('click', '.delete-btn', function() {
   ...
   $('#proceed').click(function() {
            // Check if Table 1 or Table 2 tbody has rows
            var table1HasRows = $('#dataTable1 tbody tr').length > 0;
            var table2HasRows = $('#dataTable2 tbody tr').length > 0;

            // Check the conditions based on the row presence in tables
            if (table1HasRows && table2HasRows) {
                // Both tables have rows, display both tables
                $('#dataTable1, #dataTable2, #cover, #cover1body, #cover2body, #backButton').show();
            ...
            $('.edit-btn').hide();
            $('.delete-btn').hide();
             // Hide Proceed button after click
             $(this).hide();
            } else if (table1HasRows) {
                // Only Table 1 has rows, display Table 1
                $('#dataTable1,  #cover, #cover1body, #cover2body, #backButton').show();
                $('#dataTable2').hide();
                 // Hide other elements as needed
            $('#defaultMessage').hide();
            $('#h2').hide();
            ...
            $('#dataEntryForm').hide();
             ...
             $(this).hide();
                ...
                // Only Table 2 has rows, display Table 2
                $('#dataTable1').hide();
                $('#dataTable2,  #cover, #cover1body, #cover2body, #backButton').show();
                 // Hide other elements as needed
            $('#defaultMessage').hide();
            $('#h2').hide();
            $('.h3').hide();
            ...
            $('#dataEntryForm').hide();
             ...
             $(this).hide();
                ...
                // Both tables have no rows, display no data message
                $('#dataTable1, #dataTable2').show(); // Hide both tables
                 // Hide other elements as needed
            $('#defaultMessage,  #cover, #cover1body, #cover2body').hide();
            $('#h2').show();
            $('.h3').show();
            $('#h1').show();
            $('.edit-btn').show();
            ...
            // Show action buttons (Print, PDF, Excel, Word) if either table has rows
            if (table1HasRows || table2HasRows) {
                $('button:contains("Print"), button:contains("PDF"), button:contains("Excel"), button:contains("Word")').show();
            } else {
                // Hide buttons if no rows are found in either table
...
$('button:contains("Word")').click(function() {
    ...
    saveAs(blob, `VTBL_CATALOGUE_${auctionNo}.doc`);
         ...
         // Show/hide specific content when the back button is clicked
    $('#backButton').click(function() {
        // Show data tables
        $('#dataTable1, #dataTable2').show();

        // Hide the default message and show other elements
        $('#defaultMessage, #cover').hide();
        $('#h1, #h2, .h3').show();
        $('#dataEntryForm, #proceed').show();
...
if (!auctionNoFromStorage) {
    ...
    function fetchData(auction_no) {       
        ...
        $('#startinglot').show();
        $('#starting_lot').show();
        $('#AuctionNos').text(auction_no);
        $('#secondaryAuctionNo').text(auction_no);
        ...
        $.ajax({
            ...
            success: function(data) {
               const startingLot = parseInt($('#starting_lot').val()) || '';
                ...
                const gradesTable1 = ['BP1', 'PF1', 'PD', 'D1'];
